...
def run_LMU(args, params, x_train, y_train, x_test, y_test, n_cls):
...
if __name__ == '__main__':
    ...
    try:
        ...
        nni_slmu_dict = pickle.load(open('./nni_slmu_dict.pkl', 'rb'))
        separate_datasets = pickle.load(open('./separate_datasets.pkl', 'rb'))

        #for clst in nni_slmu_dict.keys():
        run_LMU(ARGS, PARAMS, nni_slmu_dict[0]['train']['data'], nni_slmu_dict[0]['train']['labels'], 
                nni_slmu_dict[0]['test']['data'], nni_slmu_dict[0]['test']['labels'],
                len(separate_datasets[0]['classes']))
